...
// just add the new carModel to it with the produced cars as its value. If even the carModel exists, just add the given value to the current one.
// Output
...
function engineeringCompany(input) {
    ...
    for (const element of input) {
        ...
        if (!brandStorage.hasOwnProperty(brand)) {
            brandStorage[brand] = {}
        }
        if (!brandStorage[brand].hasOwnProperty(model)) {
            brandStorage[brand][model] = 0

        }
        brandStorage[brand][model] += Number(producedCars)

    }

    for (let [brand, models] of Object.entries(brandStorage)) {
        console.log(brand);
